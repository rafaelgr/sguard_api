...
module.exports.getRondaRealizadaDetalle = function(id, callback) {
    var connection = getConnection();
    var rondas = null;
    sql = "SELECT rr.*, r.nombre AS rnombre, v.nombre AS vnombre, rrp.rondaRealizadaPuntoId, rrp.orden, rrp.puntoId,";
    sql += " p.nombre AS pnombre, rrp.fecha AS pfecha, rrp.hora AS phora, rrp.resultado AS presultado";
    sql += " FROM rondas_realizadas AS rr";
    sql += " LEFT JOIN rondas AS r ON r.rondaId = rr.rondaId";
    sql += " LEFT JOIN vigilantes AS v ON v.vigilanteId = rr.vigilanteId";
...
function fromDbtoJsRondaRealizada(registros) {
    ...
    for (var i = 0; i < registros.length; i++) {
        ...
        if (!registro.rondaRealizadaPuntoId || !registro.puntoId) {
            continue;
        }
        var punto = {
...
module.exports.postRondaRealizadaDesdeTag = function(tag, fecha, hora, vigilanteId, callback) {
...
module.exports.putPuntoRondaRealizadaCarga = function(id, puntoRondaRealizada, callback) {
    var connection = getConnection();
    sql = "UPDATE rondas_realizadaspuntos SET ? WHERE id = ?";
    sql = mysql.format(sql, [rondaRealizada, rondaRealizada.rondaRealizadaId]);
    connection.query(sql, function(err, result) {
        if (err) {
            callback(err);
