...
var fnGetDescargaDetalle = function(descargaId, callback) {
    ...
    var sql = "SELECT d.descargaId, d.nterminal, d.fecha, d.hora, d.resultado, dl.descargaLineaId, dl.tag, dl.fecha AS lfecha, dl.hora AS lhora, dl.tipo, dl.tipoId, dl.nombre";
                    ...
                    case "RONDA":
                        ...
                        if (rondaRealizada) {
                            ...
                            rondaRealizadaDb.putRondaRealizadaCarga(rondaRealizada.rondaRealizadaId, rr, function(err, res) {
                                ...
                                rondaRealizadaDb.postRondaRealizadaDesdeTag(lectura.tag, lectura.fecha, lectura.hora, vigilanteId, function(err, res) {
                                    if (err) {
                                        logMens += "ERROR: " + err.Message + "<br/>";
                                        callback2(err);
                                        return;
                            ...
                            // no hay ronda previa (creamos una nueva)
                            rondaRealizadaDb.postRondaRealizadaDesdeTag(lectura.tag, lectura.fecha, lectura.hora, vigilanteId, function(err, res2) {
                                if (err) {
                                    logMens += "ERROR: " + err.Message + "<br/>";
                                    callback2(err);
                                    return;
                    ...
                    case "PUNTO":
                        ...
                        if (!rondaRealizada) {
                            ...
                            rondaRealizadaDb.postRondaRealizada(rr, function(err, res2) {
                                ...
                                rondaRealizada = res2;
                                logMens += "** CREADA RONDA REALIZADA DESCONOCIDA ID:" + rondaRealizada.rondaRealizadaId + "<br/>";
                                ordenEnRonda = 0;
                                fnProcesarPuntoLeido(lectura, rondaRealizada, function(err) {
                                    if (err) {
...
var fnProcesarPuntoLeido = function(lectura, rondaRealizada, callback) {
    ...
    rondaRealizadaDb.getPuntoRondaRealizadaCarga(id, lectura.tipoId, function(err, res) {
        ...
        if (puntos.length > 0) {
            ...
            if (punto.orden == ordenEnRonda) {
                ...
                logMens += "PUNTO ID:" + lectura.tipoId + " ENCONTRADO EN SECUENCIA CORRECTA" + "<br/>";
            } else {
                prr.resultado = "FUERA DE SECUENCIA";
                resultadoRonda = "PUNTOS FUERA DE SECUENCIA";
                logMens += "PUNTO ID:" + lectura.tipoId + " ENCONTRADO FUERA DE SECUENCIA" + "<br/>";
            }
            ...
            rondaRealizadaDb.postPuntoRondaRealizada(prr, function(err, res) {
                ...
                logMens += "*************** PUNTO ID:" + lectura.tipoId + " NO PERTENECE A ESTA RONDA " + "<br/>";
                callback();
                return;
            });
        }
